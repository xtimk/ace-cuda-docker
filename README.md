...
### RTX 2000+ / Ada and later — macOS Apple Silicon (cross-compile to amd64)

Builds via QEMU emulation — correct output but slower than a native amd64 build.
...
cu128 dropped sm_61 (Pascal), so PyTorch must be overridden. torch, torchaudio, and torchvision must always be the same release.
...
docker run --rm --runtime=nvidia --gpus all ^
    -p 127.0.0.1:7860:7860 ^
    -p 127.0.0.1:8888:8888 ^
    -v C:\path\to\workspace:/workspace ^
    tizm/ace-cuda:1.2.0-cu12.8.1-py3.11-u22.04
```
